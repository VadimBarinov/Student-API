...
from app.routes.lessons.rb import RBLessonDep
...
async def get_all_lessons_by_filter(session: SessionDep, lesson: RBLessonDep) -> list[SLessonGet] | dict:
    check_lesson = await LessonDAO.find_lessons(session=session, **lesson.to_dict())
...
async def get_lesson_by_id(session: SessionDep, lesson_id: int) -> SLessonGet | dict:
    check_lesson = await LessonDAO.find_all_data(session=session, lesson_id=lesson_id)
...
async def add_new_lesson(session: SessionDep, lesson: SLessonAdd) -> dict:
    check_lesson = await LessonDAO.add(session=session, **lesson.model_dump())
    if check_lesson:
...
async def update_lesson_auditorium(session: SessionDep, lesson: SLessonUpdateAuditorium) -> dict:
...
async def delete_lesson_by_id(session: SessionDep, lesson_id: int) -> dict:
    check_lesson = await LessonDAO.delete(session=session, id=lesson_id)
    if check_lesson:
